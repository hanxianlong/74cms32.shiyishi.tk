...
elseif ($act=="dynamic")
{
		...
		{
		$html[]="<li><a href=".url_rewrite('QS_companyshow',array('id0'=>$row['company_id'],'addtime'=>$row['company_addtime']))." target=\"_blank\">".cut_str($row['companyname'],5,0,"...")."</a> 发布了职位:<a href=".url_rewrite('QS_jobsshow',array('id0'=>$row['id'],'addtime'=>$row['addtime']))." target=\"_blank\">".$row['jobs_name']."</a></li>";
		}
		...
		$html[]="<li><a href=".url_rewrite('QS_resumeshow',array('id0'=>$row['id'],'addtime'=>$row['addtime']))." target=\"_blank\">".$row['fullname']." </a> 发布了简历</li>";
		}
...
elseif ($act=="company_down_resume")
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		$html[$row['bid']]="<li><a href=".url_rewrite('QS_companyshow',array('id0'=>$row['bid'],'addtime'=>$row['baddtime']))." target=\"_blank\">".$row['companyname']." </a> 下载了 <a href=".url_rewrite('QS_resumeshow',array('id0'=>$row['cid'],'addtime'=>$row['caddtime']))." target=\"_blank\">".$row['fullname']." </a>的个人简历<span>{$row['time']}</span></li>";
		}
	}
	exit(implode("",$html));
}
