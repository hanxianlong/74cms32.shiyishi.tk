...
function distribution_resume($id,$uid)
{
	...
	foreach($id as $v)
	{
		...
		{
				$j=!empty($t1)?$t1:$t2;
				if (!empty($t1) &&  $t1['audit']=="1" && $t1['display']=="1" && $t1['user_status']=="1" && $t1['complete']=="1")
				{
					continue;
				}
				if (!empty($t2) && ($t2['audit']!="1" || $t2['display']!="1" || $t2['user_status']!="1"  || $t2['complete']!="1"))
				{
					continue;
				}
...
function get_notice($offset,$perpage,$get_sql='',$titlelen=12)
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		$row['url'] =$row['is_url']<>"http://"?$row['is_url']:"personal_notice.php?act=notice_show&id=".$row['id'];
		$List[] = $row;
	}
	return $List;
}
