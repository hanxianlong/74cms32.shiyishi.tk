...
if (!empty($uidarr))
{
	...
	{		
		...
		$companyarray[$row['uid']]['company_url']=url_rewrite($aset['companyshow'],array('id0'=>$row['company_id'],'addtime'=>$row['company_addtime']));
		...
		$companyarray[$row['uid']]['jobs'][$row['id']]['jobs_url']=url_rewrite($aset['jobsshow'],array('id0'=>$row['id'],'addtime'=>$row['addtime']));
