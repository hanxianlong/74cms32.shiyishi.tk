function tagso(templatedir,dir,jobslisturl,getarr)
{
	...
	opthtm+='<input name="jobtag" type="hidden" value="'+get[4]+'" />';
	...
	if (get[4]=='')
	{
		...
		{
		arr    =QS_jobtag[i].split(",");
		opthtm+='<li><a href="'+href+'" id="jobtag-'+arr[0]+'"  class="opt">'+arr[1]+'</a></li>';
		}
	}
	else
	{
		...
		if (scalename)
		{
		opthtm+='<li><span  id="jobtag-'+get[4]+'">'+scalename+'</span></li>';	
		}
		else
		{
			alert('参数错误');
	...
	if (selopt!='')
	{
		...
		if (get[4])	{
		var optval=$('#jobtag-'+get[4]).html();
		selbox.append('<a href="'+href+'" class="clearopt" id="jobtag-'+get[4]+'" title="点击取消"><u>标签:</u>'+optval+'</a>');
		}
		if (get[0])	{
