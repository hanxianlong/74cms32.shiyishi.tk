...
elseif ($act=='editjobs_save')
{
	...
	check_permissions($_SESSION['admin_purview'],"jobs_edit");
	$id=intval($_POST['id']);
	$company_profile=get_company_one_id($id);
...
elseif($act == 'edit_setmeal_save')
{
	...
	if ($company_uid)
	{
			...
			if ($setsqlarr['endtime']<>"")
			{
				...
				if (!updatetable(table('jobs'),$setmeal_deadline," uid={$company_uid}"))adminmsg('修改出错！',0);
				if (!updatetable(table('jobs_tmp'),$setmeal_deadline," uid={$company_uid}"))adminmsg('修改出错！',0);
