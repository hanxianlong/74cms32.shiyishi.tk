...
function get_jobs_one($id)
{
	...
	$val['jobs_url']=url_rewrite('QS_jobsshow',array('id0'=>$val['id'],'addtime'=>$val['addtime']));
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		while($row = $db->fetch_array($result))
		{
			...
			else
			{
				...
				if (!$db->query("update  ".table('jobs_tmp')." SET setmeal_deadline='{$deadline}'  WHERE id='{$row['uid']}'  LIMIT 1")) return false;
			}
		}
	}
	return $return;
	 ...
	 if (!empty($uid) && is_array($uid))
	 {
	 	foreach($uid as $vuid)
		{
			...
			if ($gift=='companyauth')
			{
				...
				write_memberslog($com['uid'],1,9001,$user['username']," 成为已认证企业({$operator}{$points})，(剩余:{$mypoints})");
...
function get_promotion($offset,$perpage,$get_sql= '')
{
	...
	$row['jobs_url']=url_rewrite('QS_jobsshow',array('id0'=>$row['cp_jobid'],'addtime'=>$row['addtime']));
