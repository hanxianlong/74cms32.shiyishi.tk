...
elseif($act == 'site_navigation_add_save')
{
	...
	!inserttable(table('navigation'),$setsqlarr)?adminmsg("添加失败！",0):adminmsg("添加成功！",2,$link);
}
elseif($act == 'del_navigation')
{
	...
	!del_navigation($id)?adminmsg("删除失败！",0):adminmsg("删除成功！",2,$link);
}
...
elseif($act == 'site_navigation_edit_save')
{
	...
	!updatetable(table('navigation'),$setsqlarr,$wheresql)?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
...
{
	check_token();
	!del_nav_cat(intval($_GET['id']))?adminmsg("删除失败！",0):adminmsg("删除成功！",2,$link);
}
