...
$login_allback="{$_CFG['site_domain']}{$_CFG['site_dir']}user/qqconnect.php?act=login_allback" ;
$binding_callback="{$_CFG['site_domain']}{$_CFG['site_dir']}user/qqconnect.php?act=binding_callback" ;
...
elseif ($act=='reg')
{
	...
	{
		require_once(QISHI_ROOT_PATH.'include/tpl.inc.php');
		$smarty->assign('title','补充注册 - '.$_CFG['site_name']);
		$smarty->assign('qqurl',"?act=");
		$smarty->display('user/qqconnect.htm');
	}
}
elseif ($act=='reg_save')
{
	...
	unset($dbhost,$dbuser,$dbpass,$dbname);
	require_once(QISHI_ROOT_PATH.'include/fun_user.php');
	$username=user_register($val['username'],$val['password'],$val['member_type'],$val['email']);
	if ($username)
	{
		$user=get_user_inusername($username);
		update_user_info($user['uid']);
		header("Location:{$_CFG['site_domain']}{$_CFG['site_dir']}");
	}
